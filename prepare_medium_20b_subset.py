#!/usr/bin/env python3
"""
Prepare medium subset (20,000 samples) from AgentGym SFT trajectories dataset
for 20B model training with stratified sampling across environments
"""
...
def create_stratified_subset():
    """Download dataset and create 20,000 sample subset with stratified sampling"""
    ...
    # Calculate target samples per environment (total: 20,000)
    target_total = 20000
    target_distribution = {
        'alfworld': int(target_total * 0.28),      # 5,600 samples (28%)
        'webshop': int(target_total * 0.24),       # 4,800 samples (24%)
        'babyai': int(target_total * 0.20),        # 4,000 samples (20%)
        'sciworld': int(target_total * 0.18),      # 3,600 samples (18%)
        'textcraft': int(target_total * 0.10),     # 2,000 samples (10%)
    }

    print(f"\n🎯 Target distribution for 20K sample subset:")
    ...
    print(f"\n🎯 20K subset created with {len(subset_data)} samples")
    ...
    # Quality filtering (slightly more lenient for larger model)
    ...
    for item in axolotl_data:
        ...
        # Slightly wider range for 20B model (300 to 10000 characters)
        if 300 <= total_chars <= 10000:
            filtered_data.append(item)

    print(f"   Length filter: {len(axolotl_data)} -> {len(filtered_data)} samples")

    # Ensure we have exactly 20,000 samples (or as close as possible)
    ...
    output_file = Path("agentgym_medium_20b_subset.jsonl")
    with open(output_file, 'w', encoding='utf-8') as f:
        for item in filtered_data:
            f.write(json.dumps(item) + '\n')

    print(f"\n✅ 20B model dataset saved to: {output_file}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"\n🚀 Ready for 20B model training!")
        print(f"   Use this dataset file: {subset_file}")
        print(f"   Training command: axolotl train gpt-oss-20b-axolotl-config.yml")
